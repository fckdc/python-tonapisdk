...
class Validators(BaseModel):
    ...
    Validators
    """
    validators: conlist(Validator) = Field(...)
    __properties = ["validators"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Validators:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> Validators:
        """Create an instance of Validators from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Validators.parse_obj(obj)

        _obj = Validators.parse_obj({
            "validators": [Validator.from_dict(_item) for _item in obj.get("validators")] if obj.get("validators") is not None else None
        })
        return _obj
