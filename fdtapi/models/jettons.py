...
class Jettons(BaseModel):
    ...
    Jettons
    """
    jettons: conlist(JettonInfo) = Field(...)
    __properties = ["jettons"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Jettons:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> Jettons:
        """Create an instance of Jettons from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return Jettons.parse_obj(obj)

        _obj = Jettons.parse_obj({
            "jettons": [JettonInfo.from_dict(_item) for _item in obj.get("jettons")] if obj.get("jettons") is not None else None
        })
        return _obj
